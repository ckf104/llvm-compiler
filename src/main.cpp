...
// can't define function with duplicated name, a feasible idea reserving
// syntax tree for each defined function may be feasible.
...
class PrototypeAST {
 ...
 public:
  PrototypeAST(const std::string &Name, std::vector<std::string> Args,
               bool is_op = false, unsigned precedence = 0)
      : Name(Name),
        Args(std::move(Args)),
        IsOperator(is_op),
...
static std::unique_ptr<CGSCCAnalysisManager> TheSCCAM;
static std::unique_ptr<ModuleAnalysisManager> TheMAM;
static std::unique_ptr<PassBuilder> PBuilder;
...
Function *getFunction(std::string Name) {
  ...
  // assert(TheModule->empty());
  if (auto *F = TheModule->getFunction(Name))  // example: recursive
    return F;
...
Value *ForExprAST::codegen() {
  ...
  // Compute the end condition, before step operation
...
Value *VarExprAST::codegen() {
  ...
  for (auto iter = NamedValues.begin(); iter != NamedValues.end();) {
    if (!iter->second)
      iter = NamedValues.erase(iter);
    else
      ++iter;
  }
...
Value *IfExprAST::codegen() {
  ...
  // it's possible there are others basicblocks between then and else.
...
void PrototypeAST::setArgs(Function *F) {
  unsigned Idx = 0;
  for (auto &Arg : F->args()) Arg.setName(Args[Idx++]);
}
...
static void InitializeModule() {
  ...
  PBuilder = std::make_unique<PassBuilder>();
  TheFAM = std::make_unique<FunctionAnalysisManager>();
  TheMAM = std::make_unique<ModuleAnalysisManager>();
  PBuilder->registerFunctionAnalyses(*TheFAM);
  PBuilder->registerModuleAnalyses(*TheMAM);
  TheFAM->registerPass(
      []() { return ModuleAnalysisManagerFunctionProxy(*TheMAM); });
...
static void HandleDefinition() {
  if (auto FnAST = ParseDefinition()) {
    if (auto *FnIR = FnAST->codegen()) {
      ...
      fprintf(stderr, "\n");
      if (JITmode) {
        ExitOnErr(TheJIT->addModule(orc::ThreadSafeModule(
...
int main() {
  ...
  if (JITmode) TheJIT = ExitOnErr(orc::KaleidoscopeJIT::Create());
